 ...
 * @package Xfers
 */
abstract class ApiResource extends XfersObject
{
    private static $HEADERS_TO_PERSIST = array('Xfers-Account' => true, 'Xfers-Version' => true);
    ...
    public static function className()
    {
        ...
        if (substr($class, 0, strlen('Xfers')) == 'Xfers') {
            $class = substr($class, strlen('Xfers'));
        }
    ...
    private static function _validateParams($params = null)
    {
        if ($params && !is_array($params)) {
            $message = "You must pass an array as the first argument to Xfers API "
                . "method calls.  (HINT: an example call to create a charge "
                . "would be: \"Xfers\\Charge::create(array('amount' => 100, "
    ...
    protected static function _all($params = null, $options = null)
    {
        ...
        $obj = Util\Util::convertToXfersObject($response->json, $opts);
        if (!is_a($obj, 'Xfers\\Collection')) {
            $class = get_class($obj);
            $message = "Expected type \"Xfers\\Collection\", got \"$class\" instead";
            throw new Error\Api($message);
        }
